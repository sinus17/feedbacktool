...
CREATE TABLE public.whatsapp_logs (
  id TEXT PRIMARY KEY DEFAULT LPAD(FLOOR(RANDOM() * 100000000)::TEXT, 8, '0'),
  created_at TIMESTAMP WITH TIME ZONE DEFAULT timezone('utc'::text, now()) NOT NULL,
  type TEXT NOT NULL CHECK (type IN ('feedback', 'video_deleted', 'video_renamed', 'type_changed', 'group_description', 'feedback_reminder', 'correction_reminder', 'group_access', 'message_deleted')),
...
CREATE POLICY "Public insert access" ON public.artists FOR INSERT WITH CHECK (true);
CREATE POLICY "Public update access" ON public.artists FOR UPDATE USING (true);
CREATE POLICY "Public delete access" ON public.artists FOR DELETE USING (true);

CREATE POLICY "Public read access" ON public.submissions FOR SELECT USING (true);
CREATE POLICY "Public insert access" ON public.submissions FOR INSERT WITH CHECK (true);
CREATE POLICY "Public update access" ON public.submissions FOR UPDATE USING (true);
CREATE POLICY "Public delete access" ON public.submissions FOR DELETE USING (true);
...
CREATE POLICY "Public update access" ON public.messages FOR UPDATE USING (true);
CREATE POLICY "Public delete access" ON public.messages FOR DELETE USING (true);

CREATE POLICY "Public read access" ON public.whatsapp_logs FOR SELECT USING (true);
CREATE POLICY "Public insert access" ON public.whatsapp_logs FOR INSERT WITH CHECK (true);
